...
Na receita [Navegar para uma nova tela e voltar][Navigate to a new screen and back],
você aprendeu como navegar para uma nova tela criando uma nova rota e
enviando-a para o [`Navigator`][`Navigator`].
...
use a função [`Navigator.pushNamed()`][`Navigator.pushNamed()`].
...
ao construtor [`MaterialApp`][`MaterialApp`]: a `initialRoute`
e as `routes` em si.

A propriedade `initialRoute` define com qual rota o app deve iniciar.
...
Com os widgets e rotas em vigor, acione a navegação usando o
método [`Navigator.pushNamed()`][`Navigator.pushNamed()`].
...
função [`Navigator.pop()`][`Navigator.pop()`].
