...
Não é suficiente apenas adaptar como seu app se parece,
você também tem que suportar uma variedade de entradas do usuário.
O mouse e o teclado introduzem tipos de entrada além daqueles
encontrados em um dispositivo touch, como roda de rolagem, clique direito,
interações hover, navegação por tab, e atalhos de teclado.

Alguns desses recursos funcionam por padrão em widgets Material.
Mas, se você criou um widget customizado,
...
alguns recursos, como navegação por tab e atalhos de teclado,
são _críticos para usuários que trabalham com dispositivos assistivos_.
Além dos conselhos padrão para
...
## Navegação por tab e interações de foco

Usuários com teclados físicos esperam que possam usar
a tecla tab para navegar rapidamente por uma aplicação,
e usuários com diferenças motoras ou visuais frequentemente dependem
...
Se você tem seu próprio widget que quer incluir na
travessia, pode usar o widget [`FocusableActionDetector`][`FocusableActionDetector`]
para criar seus próprios controles. O [`FocusableActionDetector`][`FocusableActionDetector`]
...
Para obter mais controle sobre a ordem em que
os widgets são focados quando o usuário pressiona tab,
você pode usar [`FocusTraversalGroup`][`FocusTraversalGroup`] para definir seções
da árvore que devem ser tratadas como um grupo ao navegar por tab.

Por exemplo, você pode querer navegar por tab através de todos os campos em
um formulário antes de navegar para o botão submit:
...
Flutter tem várias maneiras integradas de percorrer widgets e grupos,
padronizando para a classe `ReadingOrderTraversalPolicy`.
...
Além da navegação por tab, usuários de desktop e web estão acostumados
...
já tem um nó de foco, você pode envolvê-lo em um [`KeyboardListener`][`KeyboardListener`]
ou um widget [`Focus`][`Focus`] e escutar eventos de teclado:
...
da árvore, use o widget [`Shortcuts`][`Shortcuts`]:
...
O widget [`Shortcuts`][`Shortcuts`] é útil porque apenas
...
é fácil com [`HardwareKeyboard`][`HardwareKeyboard`]:
...
está digitando em um campo ou quando o widget ao qual está
associado está oculto da visualização.
Ao contrário de `Shortcuts` ou `KeyboardListener`,
essa é sua responsabilidade gerenciar. Isso pode ser especialmente
...
o mouse está pairando. Por exemplo, você tipicamente vê
...
Se você mudar os estilos de botão Material padrão,
...
`MouseRegion` também é útil para criar efeitos customizados
de rollover e hover:
...
para contornar o botão quando ele tem foco,
confira o [código do botão para o app Wonderous][button code for the Wonderous app].
O app modifica a propriedade [`FocusNode.hasFocus`][`FocusNode.hasFocus`]
...
de um widget para acomodar uma tela touch, por exemplo.

Diferentes dispositivos de entrada oferecem vários níveis de precisão,
o que necessita áreas de toque de tamanhos diferentes.
...
que a suportam animam suas densidades para corresponder.
Por padrão, ambas as densidades horizontal e vertical
estão definidas como 0.0, mas você pode definir as densidades para qualquer
valor negativo ou positivo que quiser.
Ao alternar entre diferentes
densidades, você pode facilmente ajustar sua UI.
...
Não só o container reage automaticamente a mudanças
em densidade, mas também anima quando muda.
Isso une seus componentes customizados,
juntamente com os componentes integrados,
...
mas isso é totalmente sua decisão.
O fato de não ter unidade a torna bastante versátil,
...
componentes suportados, consulte a API [`VisualDensity`][`VisualDensity`].
