...
customizada entre telas pode tornar um app mais único. Para ajudar,
[`PageRouteBuilder`][] fornece um objeto [`Animation`].
Esta `Animation` pode ser usada com objetos [`Tween`][] e
[`Curve`][] para customizar a animação de transição.
Esta receita mostra como transicionar entre
rotas animando a nova rota para a visualização a partir
da parte inferior da tela.

Para criar uma transição de rota de página customizada, esta receita usa os seguintes passos:
...
O exemplo a seguir cria duas rotas: uma rota home com um botão "Go!", e
...
Definir o argumento `dy` como 1 representa uma translação vertical de uma
altura completa da página.

O callback `transitionsBuilder` tem um parâmetro `animation`. É uma
`Animation<double>` que produz valores entre 0 e 1. Converta a
`Animation<double>` em uma `Animation<Offset>` usando um Tween:
...
SlideTransition recebe uma `Animation<Offset>` e translada seu filho (usando um
...
Flutter fornece uma seleção de curvas de atenuação que
...
Em seguida, use este tween passando-o para `animation.drive()`. Isso cria uma nova
`Animation<Offset>` que pode ser dada ao widget `SlideTransition`:
...
`CurveTween`, depois avaliando o `Tween<Offset>.` Quando a animação executa, os
valores são computados nesta ordem:

1. A animation (fornecida ao callback transitionsBuilder) produz valores
   de 0 a 1.
...
Outra maneira de criar uma `Animation<Offset>` com uma curva de atenuação é usar uma
`CurvedAnimation`:
