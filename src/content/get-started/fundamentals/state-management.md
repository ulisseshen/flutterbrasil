...
para acessar esses objetos de forma mais eficaz
...
* **Encapsulamento**
: O widget que usa `MyCounter` não tem visibilidade da
  variável `count` subjacente
  e não tem meios de acessá-la ou mudá-la.
...
* Para **atualizar** o estado compartilhado e notificar outras partes do app
* Para **ouvir** mudanças no estado compartilhado
  e reconstruir a UI quando ele muda

Esta seção explora como você pode efetivamente compartilhar estado
entre diferentes widgets no seu app.
...
e muitos frameworks tiram vantagem dele ou fornecem ferramentas para facilitar.
...
para que quaisquer widgets que dependam deste `InheritedWidget` reconstruam
quando este widget reconstruir com novos dados
e `updateShouldNotify` retornar true.
...
Para mais informações sobre compartilhamento de estado entre widgets,
...
Agora que você escolheu como quer compartilhar estado no seu app,
...
Então passe-o para `ListenableBuilder`
...
Então use o campo `value` para ler ou atualizar o valor,
e notificar quaisquer listeners de que o valor mudou.
Porque `ValueNotifier` estende `ChangeNotifier`,
...
os objetos com estado no nosso app.
...
fazer caching de dados, ou gerenciar recursos do sistema como um plugin.
...
sobre a plataforma em que está rodando;
...
e define fronteiras claras entre os componentes de baixo nível do seu app e os
componentes de UI de nível mais alto necessários para construir o app completo.

A classe `CounterData` define a estrutura dos dados
e é o verdadeiro "modelo" da nossa aplicação.
A camada model é tipicamente responsável pelos algoritmos principais
e estruturas de dados necessárias para seu app.
Se você está interessado em outras maneiras de definir o modelo,
...
Um `ViewModel` liga a _View_ ao _Model_.
...
Note que o `ViewModel` armazena um `errorMessage`
quando recebe um erro do Model.
Isso protege a View de erros de runtime não tratados,
...
Esta página toca a superfície do gerenciamento de estado já que
