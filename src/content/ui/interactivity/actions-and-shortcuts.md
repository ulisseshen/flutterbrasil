...
Esta página descreve como vincular eventos de teclado físico a ações na interface
do usuário. Por exemplo, para definir atalhos de teclado na sua aplicação, esta
página é para você.

## Visão geral

Para uma aplicação GUI fazer algo, ela precisa ter ações: os usuários querem dizer
à aplicação para _fazer_ algo. As ações são frequentemente funções simples que
executam a ação diretamente (como definir um valor ou salvar um arquivo). Em uma
aplicação maior, no entanto, as coisas são mais complexas: o código para invocar a
ação e o código para a ação em si podem precisar estar em lugares diferentes.
Os atalhos (key bindings) podem precisar de definição em um nível que não sabe nada
sobre as ações que eles invocam.

É aí que entra o sistema de actions e shortcuts do Flutter. Ele permite que
desenvolvedores definam ações que cumprem intents vinculados a elas. Neste
contexto, um intent é uma ação genérica que o usuário deseja executar, e uma
instância da classe [`Intent`][`Intent`] representa esses intents do usuário no Flutter. Um
`Intent` pode ser de propósito geral, cumprido por diferentes ações em diferentes
contextos. Uma [`Action`][`Action`] pode ser um simples callback (como no caso do
[`CallbackAction`][`CallbackAction`]) ou algo mais complexo que se integra com arquiteturas
completas de desfazer/refazer (por exemplo) ou outra lógica.

![Using Shortcuts Diagram][Using Shortcuts Diagram]{:width="100%" .diagram-wrap}

[`Shortcuts`][`Shortcuts`] são key bindings que ativam ao pressionar uma tecla ou combinação
de teclas. As combinações de teclas residem em uma tabela com seu intent vinculado.
Quando o widget `Shortcuts` as invoca, ele envia seu intent correspondente para o
...
Você pode se perguntar: por que não apenas mapear uma combinação de teclas diretamente para uma ação? Por que
ter intents? Isso ocorre porque é útil ter uma separação de
preocupações entre onde as definições de mapeamento de teclas estão (frequentemente em um nível alto),
e onde as definições de ação estão (frequentemente em um nível baixo), e porque é
importante poder ter uma única combinação de teclas mapeada para uma operação pretendida
...
Por exemplo, Flutter tem um widget `ActivateIntent` que mapeia cada tipo de
controle para sua versão correspondente de um `ActivateAction` (e que executa
o código que ativa o controle). Este código frequentemente precisa de acesso bastante privado
...
aos atalhos ter mais conhecimento do que o necessário sobre qual ação
invocar, e ter acesso ou fornecer estado que não necessariamente teria
...
o foco, permitindo que o `DirectionalFocusAction` saiba em qual direção mover
o foco. Apenas tenha cuidado: não passe estado no `Intent` que se aplica
a todas as invocações de uma `Action`: esse tipo de estado deve ser passado para o
construtor da própria `Action`, para manter o `Intent` sem precisar saber
demais.

### Por que não usar callbacks?

Você também pode se perguntar: por que não apenas usar um callback em vez de um objeto `Action`?
A razão principal é que é útil para as ações decidirem se elas
estão habilitadas implementando `isEnabled`. Além disso, é frequentemente útil se os key
bindings e a implementação desses bindings estiverem em lugares diferentes.

Se tudo o que você precisa são callbacks sem a flexibilidade de `Actions` e
`Shortcuts`, você pode usar o widget [`CallbackShortcuts`][`CallbackShortcuts`]:
...
Como você verá abaixo, as actions são úteis por si só, mas o caso de uso mais comum
envolve vinculá-las a um atalho de teclado. Isso é para o que o widget `Shortcuts`
serve.
...
esse propósito pretendido para a combinação de teclas em uma ação concreta, o
widget `Actions` é usado para mapear o `Intent` para uma `Action`. Por exemplo, você pode
definir um `SelectAllIntent` e vinculá-lo ao seu próprio `SelectAllAction` ou ao seu
`CanvasSelectAllAction`, e a partir desse único key binding, o sistema invoca
qualquer um, dependendo de qual parte da sua aplicação tem foco. Vamos ver como
a parte de key binding funciona:
...
`ShortcutActivator`, veja nota abaixo) para uma instância de `Intent`. O logical key
set define um conjunto de uma ou mais teclas, e o intent indica o propósito pretendido
do pressionamento de tecla. O widget `Shortcuts` procura pressionamentos de teclas no mapa,
para encontrar uma instância de `Intent`, que ele fornece ao método `invoke()` da action.

:::note
`ShortcutActivator` é uma substituição para `LogicalKeySet`.
Ele permite uma ativação mais flexível e correta de atalhos.
...
Então há `CharacterActivator`, que ativa um atalho com base no
caractere produzido por uma sequência de teclas, em vez das logical keys em si.
`ShortcutActivator` também é destinado a ser subclassificado para permitir
maneiras personalizadas de ativar atalhos a partir de eventos de tecla.
:::

### O ShortcutManager

O shortcut manager, um objeto mais duradouro que o widget `Shortcuts`, passa
eventos de tecla quando os recebe. Ele contém a lógica para decidir como
lidar com as teclas, a lógica para subir pela árvore para encontrar outros mapeamentos de atalho,
e mantém um mapa de combinações de teclas para intents.

Embora o comportamento padrão do `ShortcutManager` seja geralmente desejável, o
widget `Shortcuts` recebe um `ShortcutManager` que você pode subclassificar para personalizar
sua funcionalidade.

Por exemplo, se você quisesse registrar cada tecla que um widget `Shortcuts` manipulou,
você poderia criar um `LoggingShortcutManager`:
...
Agora, toda vez que o widget `Shortcuts` manipular um atalho, ele imprime o evento de tecla
e o contexto relevante.

## Actions

`Actions` permitem a definição de operações que a aplicação pode
executar invocando-as com um `Intent`. Actions podem ser habilitadas ou desabilitadas,
...
método `invoke()`. Aqui está uma ação simples que simplesmente invoca uma função no
modelo fornecido:
...
Uma vez que você tenha uma action, adicione-a à sua aplicação usando o widget [`Actions`][`Actions`],
que recebe um mapa de tipos `Intent` para `Action`s:
...
encontrar qual action invocar. Se o widget `Shortcuts` não encontrar um tipo de
intent correspondente no primeiro widget `Actions` encontrado, ele considera o próximo
widget ancestral `Actions`, e assim por diante, até atingir a raiz da árvore de widgets,
ou encontrar um tipo de intent correspondente e invocar a action correspondente.
...
disponível no `context` fornecido. Se uma não estiver disponível, ele retorna null. Se uma
`Action` associada deve estar sempre disponível, então use `find` em vez de
`maybeFind`, que lança uma exceção quando não encontra um tipo `Intent`
correspondente.

Para invocar a action (se ela existe), chame:
...
a função `Actions.handler` cria uma closure handler.
No entanto, se não tiver um mapeamento, ele retorna `null`.
Isso permite que o botão seja desabilitado se
não houver uma action habilitada que corresponda no contexto.
...
a outra action habilitada mais alta na hierarquia de widgets (se existir) uma chance de
executar.
...
serviços que um action dispatcher possa fornecer (como logging, desfazer/refazer, e
assim por diante).

### Action dispatchers

Na maioria das vezes, você só quer invocar uma action, fazer ela fazer seu trabalho, e
esquecer disso. Às vezes, no entanto, você pode querer registrar as actions executadas.

É aí que entra a substituição do `ActionDispatcher` padrão por um dispatcher personalizado.
Você passa seu `ActionDispatcher` para o widget `Actions`, e ele
...
Isso registra cada action à medida que ela é executada, assim:
...
A combinação de `Actions` e `Shortcuts` é poderosa: você pode definir
intents genéricos que mapeiam para ações específicas no nível do widget. Aqui está um app simples
