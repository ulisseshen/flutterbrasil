...
conteúdo correspondente, mas às vezes, não há uma ação primária.
...
Comece criando um novo stateful widget chamado `ExpandableFab`.
Este widget exibe o FAB primário e coordena a expansão
e colapso dos outros botões de ação. O widget recebe
parâmetros para se o `ExpandedFab` começa ou não na
posição expandida, qual é a distância máxima de cada botão de ação,
e uma lista de children. Você usará a lista mais tarde para fornecer
...
esses dois botões escalam e fazem fade entre si.
...
permitindo a aparência visual de um cross-fade à medida que o botão superior
...
widget `IgnorePointer`. Isso ocorre porque o botão de abrir sempre existe,
...
Defina um novo stateless widget chamado `ActionButton` para exibir
...
Passe algumas instâncias deste novo widget `ActionButton` para seu
`ExpandableFab`.
...
Os `ActionButton`s filhos devem voar para fora de baixo do FAB
aberto quando expandidos. Então, os `ActionButton`s filhos devem
voar de volta para baixo do FAB aberto quando colapsados.
Este movimento requer posicionamento explícito (x,y) de cada
`ActionButton` e uma `Animation` para coreografar mudanças nessas
posições (x,y) ao longo do tempo.

Introduza um `AnimationController` e uma `Animation` para
controlar a taxa na qual os vários `ActionButton`s expandem e colapsam.
...
Em seguida, introduza um novo stateless widget chamado `_ExpandingActionButton`,
...
widget `Positioned`, que posiciona o `child` em uma coordenada
(x,y) específica dentro do `Stack` circundante.
O `AnimatedBuilder` faz com que o widget `Positioned` seja reconstruído
toda vez que a animação muda. O widget `FadeTransition`
orquestra a aparição e desaparecimento de cada
...
direto de um `Stack`. Isso se deve à relação explícita
...
* Clique no FAB no canto inferior direito,
  representado com um ícone Edit.
  Ele se abre em 3 botões e é substituído por
...
* Clique no botão de fechar para ver os botões expandidos
  ...
  o **X** é substituído pelo ícone Edit.
